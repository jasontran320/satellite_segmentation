...
def process_viirs_filename(file_path: Path) -> Tuple[str, str]:
    ...
    date_str = file_name.split("_")[2][1:-4]
...
def process_s1_filename(file_path: Path) -> Tuple[str, str]:
    ...
    band = file_name.split("_")[4].split(".")[0]
    return date, band
...
def process_s2_filename(file_path: Path) -> Tuple[str, str]:
    ...
    date = datetime.strptime(date_str, "%Y%m%d").strftime("%Y-%m-%d")
    band = file_name.split("_")[2].split(".")[0][1:]
    return date, band
...
def process_landsat_filename(file_path: Path) -> Tuple[str, str]:
    ...
    date = datetime.strptime(date_str, "%Y-%m-%d").strftime("%Y-%m-%d")
    band = file_name.split("_")[3].split(".")[0][1:]
    return date, band
...
def get_satellite_files(tile_dir: Path, satellite_type: SatelliteType) -> List[Path]:
    ...
    files = [file_path for file_path in tile_dir.rglob(file_pattern) if file_path.is_file() and file_path.suffix == '.tif']
    return files
...
def get_unique_dates_and_bands(
    ...
    for path in satellite_files:
        date, band = grouping_function(path)
        dates.add(date)
        bands.add(band)
...
def load_satellite(tile_dir: Path, satellite_type: SatelliteType) -> xr.DataArray:
    ...
    for date in dates:
        ...
        for band in bands:
            ...
            for file_name in file_names:
                ...
                file_date, file_band = grouping_function(file_name)
    ...
    data_array = xr.DataArray(
        ...
        coords = {"date": dates, "band": bands,
                  "height": range(data.shape[-2]),
                  "width": range(data.shape[-1])},
    )
    #
    # For dims:
...
def load_satellite_list(
    ...
    for satellite_type in satellite_type_list:
        data_arrays.append(load_satellite(tile_dir, satellite_type))
    return data_arrays
...
def create_satellite_dataset_list(
    ...
    for i, satellite_type in enumerate(satellite_type_list):
    # you should have an index and a data_dict in the for loop
        ...
        my_dataset = xr.Dataset(data_vars = data_dict)
        # set the dataset attribute "satellite_type" to be the value of the satellite_type_list[index]
        my_dataset.attrs["satellite_type"] = satellite_type.value

...
def gaussian_filter(data_array: xr.DataArray, sigma: float = 1) -> xr.DataArray:
    ...
    for date in range(data_array.shape[0]):
        ...
        for band in range(data_array.shape[1]):
            # apply the scipy.ndimage gaussian_filter function to the data_array[date][band] + #pass sigma
            filtered_data_array[date, band] = ndimage.gaussian_filter(data_array[date, band], sigma=sigma)
    gaussian_filter_data_array = xr.DataArray(filtered_data_array, coords = data_array.coords, dims = data_array.dims, attrs = data_array.attrs)
    # return the data array
    return gaussian_filter_data_array
...
def quantile_clip(
    ...
    if group_by_time:
        ...
        for band in range(data_array.shape[1]):
            ...
            # iterate by date
            data_array[:, band] = np.clip(data_array[:, band], q1, q2)
    ...
    # else
        ...
        for band in range(data_array.shape[1]):
            ...
            for date in range(data_array.shape[0]):
                ...
                q1 = np.quantile(data_array[date, band], clip_quantile)
                q2 = np.quantile(data_array[date, band], 1 - clip_quantile)
                # apply clipping for each image (date, band) using q1 and q2
                data_array[date, band] = np.clip(data_array[date, band], q1, q2)
...
def minmax_scale(data_array: xr.DataArray, group_by_time=True) -> xr.DataArray:
    ...
    if group_by_time:
        ...
        for band in range(data_array.shape[1]):
            ...
            my_min = np.min(minimums)
            my_max = np.max(maximums)
            ...
            for date in range(data_array.shape[0]):
                ...
                # else
                    ...
                    data_array[date, band] = (data_array[date, band] - my_min) / (
                                my_max - my_min)
    # else
    else:
        # iterate by date
        for date in range(data_array.shape[0]):
            ...
            for band in range(data_array.shape[1]):
                ...
                image_min = np.min(data_array[date][band].values)
                image_max = np.max(data_array[date][band].values)
                ...
                # else
                    ...
                    # image (date, band). If you use np functions to do the math, you can replace the pixel_value
                    # with the image (date, band) and it will calculate it as a matrix operation.
                    data_array[date, band] = (data_array[date, band] - image_min) / (image_max - image_min)
    # return the data_array
    return data_array
...
def gammacorr(data_array: xr.DataArray, gamma: float = 2) -> xr.DataArray:
    ...
    for date in range(data_array.shape[0]):
        ...
        for band in range(data_array.shape[1]):
            ...
            # with the image (date, band) and it will calculate it as a matrix operation.
            data_array[date, band] = data_array[date, band] ** (1 / gamma)
    # return the data_array
    return data_array
...
def convert_data_to_db(data_array: xr.DataArray) -> xr.DataArray:
    ...
    converted_array = np.log10(np.where(data_array <= 0, np.nan, data_array))
    # return the data_array
    return xr.DataArray(converted_array, coords = data_array.coords, dims = data_array.dims, attrs = data_array.attrs)
...
def maxprojection_viirs(data_array: xr.DataArray) -> xr.DataArray:
    ...
    maximum = data_array.values[0][band_index]

    # iterate by date
    for date in range(data_array.sizes['date']):
        ...
        maximum = np.maximum(maximum, data_array.values[date][band_index])
    ...
    reshaped_max = np.reshape(maximum, (1, 1, 800, 800))
    max_viirs_array = xr.DataArray(data = reshaped_max,
                                   dims = ("date", "band", "height", "width"),
                                   coords = {
                                       "date": np.arange(1),
                                       "band": np.arange(1),
                                       "height": np.arange(800),
                                       "width": np.arange(800),
                                   })
...
def preprocess_sentinel1(
    ...
    """
    # convert data to db
    converted_data = convert_data_to_db(sentinel1_data_array)
    # quantile clip (make sure to pass the parameter)
    clipped_data = quantile_clip(converted_data, clip_quantile, group_by_time=True)
    # apply a gaussian_filter (make sure to pass the parameter)
    gaussian_data = gaussian_filter(clipped_data, sigma = sigma)
    # minmax and return
    minimax_data = minmax_scale(gaussian_data, group_by_time=True)
    return minimax_data
...
def preprocess_sentinel2(
    ...
    """
    # quantile clip (make sure to pass the parameter)
    clipped_data = quantile_clip(sentinel2_data_array, clip_quantile, group_by_time = True)
    # gamma correct (make sure to pass the parameter)
    gamma_data = gammacorr(clipped_data, gamma)
    # minmax and return
    minimax_data = minmax_scale(gamma_data, group_by_time = True)
    return minimax_data
...
def preprocess_landsat(
    ...
    """
    # quantile clip (make sure to pass the parameter)
    clipped_data = quantile_clip(landsat_data_array, clip_quantile, group_by_time = True)
    # gamma correct (make sure to pass the parameter)
    gamma_data = gammacorr(clipped_data, gamma)
    # minmax and return
    minimax_data = minmax_scale(gamma_data, group_by_time = True)
...
def preprocess_viirs(
    ...
    """
    # quantile clip (make sure to pass the parameter)
    clipped_data = quantile_clip(viirs_data_array, clip_quantile, group_by_time = True)
    # minmax and return
    minimax_data = minmax_scale(clipped_data, group_by_time = True)

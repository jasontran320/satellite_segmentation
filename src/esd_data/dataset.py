...
from src.preprocessing.file_utils import get_satellite_files
...
class ESDDataset(Dataset):
    ...
    def __init__(
        ...
        tiles = processed_dir / "subtiles"
        for tile in tiles.glob("*"):
            # iterate over the subtiles within the tile
            for subtile in tile.glob("*"):  # This now fetches directories like 'Tile1'
    ...
    def __aggregate_time(self, array: np.ndarray):
        ...
        return array.reshape(-1, array.shape[2], array.shape[3])
    ...
    def __getitem__(self, idx):
        ...
        subtile_dir = self.subtile_dirs[idx]
        my_subtile = Subtile.load_subtile_by_dir(subtile_dir, self.satellite_type_list)
        # get the data_array_list and ground truth from the subtile satellite list and ground truth
        data_array_list = my_subtile.satellite_list
        ground_truth = my_subtile.ground_truth
        ...
        for data in data_array_list:
            # aggregate time and append the array to X
            processed_data = self.__aggregate_time(data.values)

...
class ESDDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        if not self.processed_dir.exists() or not any(self.processed_dir.iterdir()):
            self.processed_dir.mkdir(parents = True,
                                     exist_ok = True)  # Ensure the directory is created
            ...
            for tile_dir in tqdm(tile_dirs_train, desc = "Processing train tiles"):
                ...
                satellite_list, ground_truth = self.load_and_preprocess(tile_dir)
                # create a subtile, passing the data array list, gt data array, and the slice size
                subtile = Subtile(satellite_list = satellite_list, ground_truth = ground_truth,
    ...
    def train_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        return torch.utils.data.DataLoader(
            self.train_dataset,  # The dataset from which to load the data
            batch_size = self.batch_size,  # The batch size
            collate_fn = collate_fn,  # Function to merge a list of samples to form a mini-batch
            num_workers = self.num_workers #change for gradescope
        )
    ...
    def test_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        """
        return torch.utils.data.DataLoader(
            self.test_dataset,  # The dataset from which to load the data
            batch_size = self.batch_size,  # The batch size
            collate_fn = collate_fn,  # Function to merge a list of samples to form a mini-batch
    ...
    def val_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        # and collate_fn
        return torch.utils.data.DataLoader(
            self.val_dataset,  # The dataset from which to load the data
            batch_size = self.batch_size,  # The batch size
            collate_fn = collate_fn,  # Function to merge a list of samples to form a mini-batch
            num_workers = self.num_workers
        )
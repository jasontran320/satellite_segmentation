...
class Blur(object):
    ...
    def __call__(self, sample: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        after_blur = apply_per_band(img, lambda band: cv2.blur(band, k_size))

        # return the {X : img, y : mask}
        return {"X": after_blur, "y": mask}
...
class AddNoise(object):
    ...
    def __call__(self, sample):
        ...
        after_noise = np.clip(img + noise, 0, 1)

        # return the {X : img, y : mask}
        return {"X": after_noise, "y": mask}
...
class RandomVFlip(object):
    ...
    def __call__(self, sample):
        ...
        if random.random() < self.p:
            ...
            f_img = np.array([cv2.flip(band, 0) for band in img])
            # also flip the mask (not per band, just directly on the mask)
            f_mask = cv2.flip(mask, 0)
        else:
            f_img, f_mask = img, mask
        # return the {X : img, y : mask}
        return {"X": f_img, "y": f_mask}
...
class RandomHFlip(object):
    ...
    def __call__(self, sample):
        ...
        if random.random() < self.p:
            # apply per band the cv2.flip function (you can pass it as a lambda)
            f_img = np.array([cv2.flip(band, 1) for band in img])
            # also flip the mask (not per band, just directly on the mask)
            f_mask = cv2.flip(mask, 1)
        else:
            f_img, f_mask = img, mask
        # return the {X : img, y : mask}
        return {"X": f_img, "y": f_mask}

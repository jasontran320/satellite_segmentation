...
def flatten_dataset(data_set: xr.DataArray) -> np.ndarray:
    ...
    """
    flattened_data = np.concatenate([data_set[var_name].values.flatten() for var_name in data_set.data_vars])
...
def flatten_dataset_by_band(data_set: xr.DataArray) -> np.ndarray:
    ...
    """
    list_datas = []
    for band in range(data_set.sizes['band']):
        data_bands = data_set.isel(band=band).to_array().values.flatten()
        list_datas.append(data_bands)
    flattened_data = np.stack(list_datas)
    return flattened_data
...
def plot_sentinel1_histogram(
    ...
    for band in range(num_bands):
        ...
        ax[band, 0].tick_params(labelsize = 15)
...
def plot_sentinel2_histogram(
    ...
    # iterate by band
    for band, ax_ in zip(range(num_bands), ax.flatten()):
        # set scale to log
        ax_.set_yscale('log')
        # plot histogram
        ax_.hist(flattened_dataset_list[band], bins = n_bins)
...
def plot_landsat_histogram(
    ...
    for band in range(num_bands):
        ...
        ax[band, 0].tick_params(labelsize = 9)
...
def plot_gt_histogram(
    ...
    ax[0, 0].set_title(
        f"GT Histogram (log scaled), {num_tiles} tiles",
        fontsize = 9
    )
...
def plot_gt(data_array: xr.DataArray, image_dir: Path = None) -> None:
    ...
    # set title to: "Ground Truth\n{data_array.attrs['parent_tile_id']}" # set the font size to 9
    ...
    for row in range(num_rows):
        ...
        for col in range(num_cols):
            ...
            if date_index < num_dates:
                ...
                red = data_array[date_index].sel(band = 'VH')
                # get Green by getting the VV image (.sel(band))
                green = data_array[date_index].sel(band = 'VV')
                ...
                ax[row, col].set_title(
                    f"Sentinel 1 RGB composite\n{data_array[date_index]['date'].values}",
                    fontsize = 9)
                # set the axis to off
                ax[row, col].axis('off')
            # else
...
def plot_satellite_by_bands(
    ...
    for band_index, bands in enumerate(bands_to_plot):
        # iterate by date
        for date in range(num_dates):
            ...
            for band in bands:
                ...
                data_list.append(data_array[date].sel(band = band))

            # stack and transpose the data list, then imshow
            stacked_data = np.stack(data_list)
            transposed_data = np.transpose(stacked_data)
            ax[date, band_index].imshow(transposed_data)
            # set the axis to off
            ax[date, band_index].axis('off')
            ...
            ax[date, band_index].set_title(
                f"{str(data_array.attrs['satellite_type']).capitalize()} {title_band}\n{(data_array['date'])[date].values}",
                fontsize = 9
            )
    # --- end ---

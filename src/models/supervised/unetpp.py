...
class double_conv2(nn.Module):
    '''(conv-BN-ReLU)X2 :   in_channels  , out_channels , out_channels    '''
    ...
    def __init__(self, in_channels, out_channels):
        ...
        if 0:
            self.up = nn.Sequential(
                nn.ConvTranspose3d(in_channels * 2 // 3, in_channels * 2 // 3, kernel_size=2, stride=2, padding=0),
                nn.BatchNorm3d(in_channels * 2 // 3),
                nn.ReLU(inplace=True),
            )
        else:
            self.up = nn.Upsample(scale_factor=2)
        self.conv = double_conv2(in_channels, out_channels)

    def forward(self, x1, x2):  # x1--up , x2 ---down
        x1 = self.up(x1)
        diffY = x2.size()[2] - x1.size()[2]  # Might be switched with diffY and diffX, just have to wait and see!!!!
...
class double_conv(nn.Module):
    '''(conv-BN-ReLU)X2 :   in_channels  , in_channels , out_channels    '''
...
class inconv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = double_conv_in(in_channels, out_channels)
...
class Unet_2D(nn.Module):
    def __init__(self, in_channels, out_channels, mode='train', scale_factor = 50, weight1 = 0.2, weight2 = 0.3, weight3 = 0.5):
        super(Unet_2D, self).__init__()
        self.inconv = inconv(in_channels, cc)
        ...
        self.outconv = outconv(cc, out_channels)
        self.mode = mode
        self.final_pool = nn.MaxPool2d(kernel_size = scale_factor)
        self.weight_y0 = weight1
        self.weight_y1 = weight2
        self.weight_y2 = weight3
    ...
    def forward(self, x):
        if self.mode == 'train':  # use the whole model when training
            ...
            y1 = self.outconv(x12)
            # Combine the outputs
            combined_output = self.weight_y0 * y0 + self.weight_y1 * y1 + self.weight_y2 * y2
            return self.final_pool(combined_output)
